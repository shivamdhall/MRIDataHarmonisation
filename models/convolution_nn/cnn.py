'''
Define classes and functions for training and evaluating a convolution neural network
'''
...
class Conv_net(torch.nn.Module):
    # This class is used to define the architecture of a Convolution neural network
    def __init__(self):
    ...
    def forward(self, x):
        ...
        enc2 = self.drop(self.conv2(enc1))
        enc3 = self.drop(self.conv3(enc2))
        enc4 = self.drop(self.conv4(enc3))
        enc5 = self.drop(self.conv5(enc4))
        enc6 = self.drop(self.conv6(enc5))
        enc7 = self.drop(self.conv7(enc6))
        out = self.conv8(enc7)
...
def train_net(net, trainloader, valiloader, epochs, log_interval, gpu, log_file):
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        validation_error = 0
        iteration = 0
        for i, validation_data in enumerate(valiloader): #batch processing
            iteration = i
...
def cnn_run(training_data, validation_data, testing_data_patches, testing_data_scans, affine_mat, epochs=100, train=True, restore_model=False):
    ...
    log_file = open("models/convolution_nn/logs.txt","a+")

    # Define path for storing visualisations
    viz_path =  "models/convolution_nn/visualisations"

    # Define path for storing predicted scans
    save_to = "models/convolution_nn/predictions"

    cnn_model = Conv_net()

    if restore_model == False:
        # Generate a new model
        cnn_model.normal_weight_init()

    else:
        # Restore the existing model
        cnn_model.load_state_dict(torch.load('models/convolution_nn/trained_cnn.pth'))
    ...
    if train == True:
        ...
        # Train the network
        print ("Training CNN")
        _, training_losses, validation_losses = train_net(cnn_model, trainloader, valiloader, epochs, 20, gpu, log_file)
        ...
        torch.save(cnn_model.state_dict(), 'models/convolution_nn/trained_cnn.pth') 
        print ("Generating predictions for test patches")

        # Get predictions of the test patches using the trained CNN
    ...
    # Evaluate the performance of the trained CNN on unseen scans
    ...
    # Generate predictions for the unseen test scans using the trained CNN
    print ("Generating predictions for unseen test scans")
    final_test_predictions = get_predictions(cnn_model, final_test_loader, gpu)
    # Reconstruct the predicted patches into full scans
    final_predicted_brains = reconstruct_brain(final_test_predictions, testing_data_scans[0], model="cnn")
    # Get performance statistics
    print ("Evaluating performance of CNN on unseen test scans")
